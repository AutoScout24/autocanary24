...
describe AutoCanary24::Client do
  ...
  let(:tags) { [{ tag: "value" }] }
  let(:deployment_check) { lambda { |stacks, elb, new_attached_instances| true } }
  ...
  context 'After switch' do
    ...
    describe 'when configuration of keep_inactive_stack is FALSE' do
      it 'should delete the inactive stack' do
        ...
        expect(ac24).to receive(:delete_stack).with(blue_cs.stack_name)
  ...
  context 'Rollback' do
    ...
    describe 'when switching from Blue to Green stack and new instances do not get healthy at the ELB' do
      it 'should trigger a rollback' do
        allow(ac24).to receive(:after_switch)
    ...
    describe 'when a rollback was triggered in first iteration' do
      let(:ac24) { AutoCanary24::Client.new({scaling_instance_percent: 50, keep_instances_balanced: true }) }
      let(:deployment_check) { lambda { |stacks, elb, new_attached_instances| false } }

      it 'should remove 3 already added instances from Green stack from ELB' do
        allow(ac24).to receive(:after_switch)
        expect(green_cs).to receive(:detach_instances_from_elb).with(elb, green_instances[0, 3])
    ...
    describe 'when a rollback was triggered in second iteration' do
      let(:ac24) { AutoCanary24::Client.new({scaling_instance_percent: 50, keep_instances_balanced: true }) }
      let(:deployment_check) { lambda { |stacks, elb, new_attached_instances| new_attached_instances.length < 5 } }

      it 'should remove 5 already added instances from Green stack from ELB' do
        allow(ac24).to receive(:after_switch)
      ...
      it 'should add 3 already removed instances from Blue stack to the ELB' do
        allow(ac24).to receive(:after_switch)
        allow(green_cs).to receive(:detach_instances_from_elb).with(elb, green_instances)
        expect(blue_cs).to receive(:attach_instances_to_elb_and_wait).with(elb, blue_instances[0, 3])
